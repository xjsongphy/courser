...
补退选课程名额监控工具。它在本机 Chrome 中登录选课系统，按设定间隔读取补退选列表；当符合条件的课程出现空余时，发一封邮件提醒。
...
本工具仅用于个人课程选课状态的查询与提醒，不保证其正确性、稳定性或可用性。**使用者应自行承担使用本工具带来的一切风险与后果**；作者对因使用 courser 导致的任何直接或间接后果**概不负责**。

请**合理设置并适当放大操作间隔**（如 `interval_min`、`page_delay_min/max` 等），保持接近正常人类的操作节奏；不要用本工具进行任何批量、高频或违反规定的操作。
