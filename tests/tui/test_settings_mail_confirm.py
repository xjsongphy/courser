...
覆盖：
- 邮件分组下新增动作行出现
- 首次回车进入确认窗口：底栏只提示「再次 Enter 确认发送」
...
async def main() -> int:
    ...
    async with app.run_test(size=(120, 36)) as p:
        ...
        # 首次回车 → 确认窗口：底栏只提示「再次 Enter 确认发送」
        await p.press("enter")
        await p.pause(0.2)
        assert app.sv.test_mail_armed_at is not None, "首次回车应进入确认窗口"
        body = _settings_text(app)
        assert "再次 Enter 确认发送" in body, "底栏应提示确认发送"
        assert "↑↓ 选择" not in body, "确认窗口期不应出现普通操作提示"
        ...
        body = _settings_text(app)
        assert "再次 Enter 确认发送" not in body, "发送后应恢复正常提示"
        joined = "\n".join(app.log_buf)
    ...
    async with app2.run_test(size=(120, 36)) as p2:
        ...
        assert "再次 Enter 确认发送" in _settings_text(app2)
        # 不再按键，等 2 秒超时
        await p2.pause(2.4)
        assert app2.sv.test_mail_armed_at is None, "2s 未确认应自动取消"
        body = _settings_text(app2)
        assert "再次 Enter 确认发送" not in body, "超时后应恢复正常提示"
